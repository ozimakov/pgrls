...
def test_snapshot_version_is_twenty_one_after_owner_capture() -> None:
    ...
    # added View.grants for the VIEW004 verify --mode escalation path; v22
...
def test_snapshot_view_grants_round_trip() -> None:
    # v23 per-view `grants` (VIEW004 verify --mode escalation): emitted only
    # when non-empty, round-trips losslessly, and a pre-v23 snapshot fails
    # closed to ().
    exposed = View(
        schema="public", name="reader_view", is_materialized=False,
        security_invoker=False, security_barrier=False,
        definition="SELECT * FROM read_secret()", references=(),
        security_definer_calls=("public.read_secret",),
        grants=(
            Grant(role="PUBLIC", privileges=("SELECT",)),
            Grant(role="anon", privileges=("SELECT", "INSERT")),
        ),
    )
    plain = View(
        schema="public", name="plain_view", is_materialized=False,
        security_invoker=False, security_barrier=False,
        definition="SELECT 1", references=(), security_definer_calls=(),
    )
    snap = Schema(tables=(), views=(exposed, plain)).to_snapshot()
    assert snap["views"][0]["grants"] == [
        {"role": "PUBLIC", "privileges": ["SELECT"]},
        {"role": "anon", "privileges": ["SELECT", "INSERT"]},
    ]
    # A view with the default owner-only ACL omits the key entirely.
    assert "grants" not in snap["views"][1]
    back = Schema.from_snapshot(snap)
    assert back.views[0].grants == exposed.grants
    assert back.views[1].grants == ()
    # A pre-v23 snapshot (no key on views) decodes to () so the escalation
    # prover abstains from the view-mediated finding (fail-closed).
    legacy = {
        "version": 22,
        "tables": [],
        "views": [
            {
                "schema": "public", "name": "reader_view",
                "is_materialized": False, "security_invoker": False,
                "security_barrier": False, "definition": "SELECT 1",
                "references": [], "security_definer_calls": [],
            }
        ],
    }
